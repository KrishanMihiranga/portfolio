...
export const openMenu = (): void => {
  ...
  // Uncomment if you want to animate the cursor when the nav is open
  // tl.to(".cursor", {
  //   delay: -1,
...
export const closeMenu = (): void => {
  ...
  // Uncomment if you want to animate the cursor when the nav is closed
  // tl.to(".cursor-active", {
...
export const fadeUp = (el: string | HTMLElement, delay = 0): void => {
  tl.from(el, {
    y: 150,
    duration: 1,
    delay,
...
export const boxHover = (e: React.MouseEvent<HTMLElement>): void => {
  ...
  if (window.innerWidth >= 986) {
    gsap.timeline()
      .to(target.querySelector(".link") as HTMLElement, {
        duration: 0,
        opacity: 1,
      })
      .from(target.querySelectorAll(".box-anim") as NodeListOf<HTMLElement>, {
        duration: 0.3,
        opacity: 0,
        y: 30,
        stagger: 0.1,
...
export const boxExit = (e: React.MouseEvent<HTMLElement>): void => {
  const target = e.currentTarget as HTMLElement;
  if (window.innerWidth >= 986) {
    gsap.to(target.querySelector(".link") as HTMLElement, {
      duration: 0,
      opacity: 0,
    });
  }
...
export const fadeIn = (el: string | HTMLElement): void => {
  gsap.to(el, {
    duration: 2,
    opacity: 1,
    y: -60,
...
export const fadeOut = (el: string | HTMLElement): void => {
  gsap.to(el, {
    duration: 1,
    opacity: 0,
    y: -20,
